...
function Pool(opts, sandCastleCreationOpts) {
  ...
  sandCastleCreationOpts._isPooled = true;
  this.sandCastleCreationOpts = sandCastleCreationOpts;
...
Pool.prototype.runOnSandCastle = function (nextScript, scriptGlobals, sandcastleData) {
  ...
  nextScript.on('timeout', function () {
    var oldSandcastle = sandcastleData.castle;
    oldSandcastle.sandbox.kill('SIGHUP');
    oldSandcastle.kill();

    var opts = _.extend(_this.sandCastleCreationOpts, {
      socket: oldSandcastle.socket
    });
    sandcastleData.castle = new SandCastle(opts);
    sandcastleData.running = false;
  });
